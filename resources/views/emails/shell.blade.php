...
<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:{{ $backgroundColor }};">
    <tr><td align="center" style="padding:24px 12px;">
        <table role="presentation" width="600" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:8px;max-width:600px;overflow:hidden;font-family:Arial,Helvetica,sans-serif;color:#18181b;border-top:4px solid {{ $primaryColor }};">
            {{-- Zelfde vorm als dashed-core's emails.layout: klikbaar naar de
                 site als er een site-url is, en helemaal niets als er geen
                 logo is (geen sitenaam als terugval, die belofte staat hier
                 niet). --}}
            @if($siteLogo ?? null)
                <tr><td align="center" style="padding:20px 24px;background:{{ $primaryColor }};">
                    @if($siteUrl ?? null)
                        <a href="{{ $siteUrl }}" style="display:inline-block;text-decoration:none;">
                            <img src="{{ $siteLogo }}" alt="{{ $listName }}" style="max-height:48px;display:inline-block;border:0;">
                        </a>
                    @else
                        <img src="{{ $siteLogo }}" alt="{{ $listName }}" style="max-height:48px;display:inline-block;border:0;">
                    @endif
